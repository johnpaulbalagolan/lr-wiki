...
1a) Just get alignment data going
1b) All the features we discussed today...
2) Get a trimmed down node running w/out slice -- ready for
development. Install data services on it - show it works. (This is a
question - I think this step make sense)
3) Prove out test cases on this work
4) Get docs in shape for examples and code lib
5) Get one more example data service going: maybe ratings
6) Get replicate working better (including this stuff but also pull-replicate)
7) Get publish signing on a node working
